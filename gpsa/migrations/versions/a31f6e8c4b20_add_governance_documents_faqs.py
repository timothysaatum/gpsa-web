...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("icon_name", sa.String(80)),
    ...
    op.create_table(
        ...
        sa.Column(
            "requires_authentication", sa.Boolean(), server_default=sa.text("false"), nullable=False
        ),
    ...
    for name, cols in [
        ...
        op.create_index(
            f"ix_governance_documents_{name}", "governance_documents", cols, unique=name == "slug"
        )
    op.create_table(
        "document_versions",
        sa.Column("id", sa.UUID(), server_default=sa.text("gen_random_uuid()"), nullable=False),
        ...
        sa.ForeignKeyConstraint(["document_id"], ["governance_documents.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
    ...
    op.create_table(
        ...
        sa.Column("deleted_at", sa.DateTime(timezone=True)),
        sa.ForeignKeyConstraint(["category_id"], ["faq_categories.id"], ondelete="SET NULL"),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("slug"),
    )
