...
_RESOURCES = {
    "categories": (
        DocumentCategory,
        {"name", "slug", "description", "icon_name", "display_order", "is_active"},
    ),
    "documents": (
        GovernanceDocument,
        {
...
def _clean(resource: str, incoming: dict[str, Any], allowed: set[str]) -> dict[str, Any]:
    ...
    if data.get("verification_status") and data["verification_status"] not in {
        "unverified",
        "verified",
        "superseded",
        "withdrawn",
    ...
    for key in ("category_id",):
        if data.get(key):
            try:
                ...
                raise HTTPException(422, f"{key} must be a UUID.") from exc
...
def _enforce_publish(user: CurrentUser, data: dict[str, Any]) -> None:
    if (
        data.get("status") in {PUBLISHED, "archived", "withdrawn"}
        or data.get("verification_status") in {"verified", "superseded", "withdrawn"}
    ) and user.role != UserRole.admin:
        raise HTTPException(
            403, "Only administrators can verify, publish, archive, or withdraw content."
        )
...
async def governance_page(
    ...
    categories = list(
        (
            await db.execute(
                ...
                .where(DocumentCategory.deleted_at.is_(None), DocumentCategory.is_active.is_(True))
                .order_by(DocumentCategory.display_order, DocumentCategory.name)
            )
        ).scalars()
    )
    conditions = [
        GovernanceDocument.deleted_at.is_(None),
        GovernanceDocument.status == PUBLISHED,
        GovernanceDocument.verification_status == "verified",
        GovernanceDocument.is_public.is_(True),
    ...
    query = (
        select(GovernanceDocument, DocumentCategory)
        .join(DocumentCategory, DocumentCategory.id == GovernanceDocument.category_id)
        .where(*conditions)
    )
    if category:
        query = query.where(DocumentCategory.slug == category)
    if year:
        query = query.where(GovernanceDocument.academic_year == year)
    if document_type:
        query = query.where(GovernanceDocument.document_type == document_type)
    if search:
        term = f"%{search.strip()}%"
        query = query.where(
            or_(
                GovernanceDocument.title.ilike(term),
                GovernanceDocument.description.ilike(term),
                GovernanceDocument.document_type.ilike(term),
                GovernanceDocument.academic_year.ilike(term),
                GovernanceDocument.version.ilike(term),
                DocumentCategory.name.ilike(term),
    ...
    rows = (
        await db.execute(
            query.order_by(
                GovernanceDocument.display_order,
                GovernanceDocument.publication_date.desc().nullslast(),
            )
    ...
    for document, document_category in rows:
        ...
        item["view_url"] = (
            f"/api/v1/about/governance/documents/{document.id}/view"
            if document.view_enabled
            else None
        )
        item["download_url"] = (
            f"/api/v1/about/governance/documents/{document.id}/download"
    ...
    faq_rows = (
        await db.execute(
            select(FaqEntry, FaqCategory)
            .outerjoin(FaqCategory, FaqCategory.id == FaqEntry.category_id)
            .where(FaqEntry.deleted_at.is_(None), FaqEntry.status == PUBLISHED)
            .order_by(FaqEntry.display_order, FaqEntry.question)
            .limit(12)
        )
    ).all()
    faqs = [
        {**_row(faq), "category": category_row.name if category_row else None}
        for faq, category_row in faq_rows
    ]
    return {
        "settings": settings_page.content if settings_page else {},
        "categories": [_row(x) for x in categories],
        "documents": documents,
        "pagination": {
            "page": page,
            "page_size": page_size,
...
async def view_document(document_id: uuid.UUID, db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    if not item.view_enabled:
        raise HTTPException(403, "Viewing is disabled for this document.")
    url = item.external_url or (
        await storage.presign(item.file_key, expires_in=300) if item.file_key else None
    )
    if not url:
...
async def download_document(document_id: uuid.UUID, db: Annotated[AsyncSession, Depends(get_db)]):
    item = await _public_document(document_id, db)
    if not item.download_enabled:
        raise HTTPException(403, "Downloading is disabled for this document.")
    url = item.external_url or (
        await storage.presign(item.file_key, expires_in=300) if item.file_key else None
    )
    if not url:
...
async def admin_list(resource: str, db: Annotated[AsyncSession, Depends(get_db)]):
    if resource == "versions":
        return [
            ...
            for x in (
                await db.execute(
                    select(DocumentVersion).order_by(DocumentVersion.created_at.desc()).limit(250)
                )
            ).scalars()
        ]
    if resource not in _RESOURCES:
        raise HTTPException(404, "Unknown governance resource.")
...
async def admin_create(
    ...
    if resource not in _RESOURCES:
        raise HTTPException(404, "Unknown governance resource.")
    model, allowed = _RESOURCES[resource]
    data = _clean(resource, payload.data, allowed)
    _enforce_publish(current_user, data)
    ...
    if (
        ...
        and (not data.get("is_public") or data.get("verification_status") != "verified")
    ):
    ...
    await AuditService(db).log(
        action="CREATE",
        entity_type=f"governance_{resource}",
        entity_id=item.id,
...
async def admin_update(
    ...
    if (
        ...
        and (
            ...
            or not item.is_public
            or not (item.file_key or item.external_url)
        )
    ):
        raise HTTPException(
            422,
            "Publishing requires a verified public document with an uploaded file or external URL.",
        )
    await AuditService(db).log(
        action="UPDATE",
        entity_type=f"governance_{resource}",
...
@router.delete(
    "/admin/{resource}/{item_id}", response_model=MessageResponse, dependencies=[_manager()]
)
async def admin_delete(
    ...
    if isinstance(item, GovernanceDocument) and (item.status == PUBLISHED or item.file_key):
        raise HTTPException(
            409, "Official document records must be archived or withdrawn, not deleted."
        )
    if current_user.role != UserRole.admin:
        raise HTTPException(403, "Only administrators can delete content.")
    item.deleted_at = datetime.now(UTC)
    await AuditService(db).log(
        action="DELETE", entity_type=f"governance_{resource}", entity_id=item.id, request=request
    )
    await db.commit()
    return MessageResponse(message="Record removed.")
...
async def upload_document(
    ...
    try:
        valid = validate_attachment_file(content, file.filename or "document")
    ...
    record = DocumentVersion(
        ...
        file_size_bytes=valid.size_bytes,
        checksum=checksum,
        is_current=True,
        status="draft",
    )
    ...
    document.mime_type = valid.mime_type
    document.file_extension = Path(record.file_name).suffix.lower().lstrip(".")
    document.file_size_bytes = valid.size_bytes
    ...
    await AuditService(db).log(
        ...
        new_values={"version": version, "file_name": record.file_name, "checksum": checksum},
        request=request,
    )
    await db.commit()
    return {"message": "Document version uploaded.", "version": version}
